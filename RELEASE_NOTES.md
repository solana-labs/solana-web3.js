...
- The exported `Signer` type is now `TransactionPartialSigner | TransactionModifyingSigner` from `@solana/kit`. The legacy `{publicKey, secretKey}` shape is no longer accepted by `Transaction.sign(...)`, `Transaction.partialSign(...)`, `VersionedTransaction.sign(...)`, `Connection.sendTransaction(...)`, `Connection.simulateTransaction(...)`, or `sendAndConfirmTransaction(...)`, and message-only signers (`MessagePartialSigner`) are no longer accepted for transaction signing. Transaction lifetime information is derived from the compiled message: a message whose first instruction is the System program's `AdvanceNonceAccount` instruction is signed with a durable nonce lifetime, and any other message is signed with a blockhash lifetime whose `lastValidBlockHeight` comes from the `VersionedTransaction.sign(...)` config or the legacy `Transaction`'s `lastValidBlockHeight` field, defaulting to the maximum possible value. A legacy `Transaction` with `nonceInfo` set always uses that nonce lifetime directly. Signing also now asserts the transaction is within the network size limit, which previously only failed at serialization time. Pass `Keypair` instances or other Kit signers; custom signers should implement Kit's `TransactionPartialSigner` or `TransactionModifyingSigner` shape.
