...
}: Config<TNotification>): Promise<DataPublisher<RpcSubscriptionsTransportDataEvents<TNotification>>> {
            ...
            switch (type) {
                case 'notification':
                    return notificationPublisher.on(
                        notificationKey,
                        listener as (data: RpcSubscriptionsTransportDataEvents<TNotification>['notification']) => void,
                        options,
                    );
                case 'error':
                    return channel.on(
                        'error',
                        listener as (data: RpcSubscriptionsTransportDataEvents<TNotification>['error']) => void,
                        options,
                    );
                default:
