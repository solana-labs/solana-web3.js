...
export function assertIsProgramDerivedAddress<TAddress extends string = string>(
    ...
    if (!validFormat) {
        throw new SolanaError(SOLANA_ERROR__ADDRESSES_MALFORMED_PDA);
    }
    if (value[1] < 0 || value[1] > 255) {
        throw new SolanaError(SOLANA_ERROR__ADDRESSES_PDA_BUMP_SEED_OUT_OF_RANGE, {
            bump: value[1],
        });
    }
...
async function createProgramDerivedAddress({ programAddress, seeds }: ProgramDerivedAddressInput): Promise<Address> {
    ...
    if (seeds.length > MAX_SEEDS) {
        throw new SolanaError(SOLANA_ERROR__ADDRESSES_MAX_NUMBER_OF_PDA_SEEDS_EXCEEDED, {
            actual: seeds.length,
            maxSeeds: MAX_SEEDS,
        });
    }
    ...
    const seedBytes = seeds.reduce((acc, seed, ii) => {
        ...
        if (bytes.byteLength > MAX_SEED_LENGTH) {
            throw new SolanaError(SOLANA_ERROR__ADDRESSES_MAX_PDA_SEED_LENGTH_EXCEEDED, {
    ...
    if (await compressedPointBytesAreOnCurve(addressBytes)) {
        throw new SolanaError(SOLANA_ERROR__ADDRESSES_INVALID_SEEDS_POINT_ON_CURVE);
    }
...
export async function getProgramDerivedAddress({
    ...
    while (bumpSeed > 0) {
        try {
            ...
            if (isSolanaError(e, SOLANA_ERROR__ADDRESSES_INVALID_SEEDS_POINT_ON_CURVE)) {
                bumpSeed--;
            } else {
                throw e;
            }
    ...
    throw new SolanaError(SOLANA_ERROR__ADDRESSES_FAILED_TO_FIND_VIABLE_PDA_BUMP_SEED);
}
...
export async function createAddressWithSeed({ baseAddress, programAddress, seed }: SeedInput): Promise<Address> {
    ...
    if (
        ...
        throw new SolanaError(SOLANA_ERROR__ADDRESSES_PDA_ENDS_WITH_PDA_MARKER);
    }
