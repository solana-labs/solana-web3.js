...
describe('exportKeyPolyfill', () => {
    it.each(['jwk', 'spki'] as const)('throws an unimplemented error when the format is %s', format => {
    ...
    describe('when format is `raw`', () => {
        it('throws when the key supplied is non-extractable', () => {
            ...
            expect(() => exportKeyPolyfill('raw', mockKey)).toThrow('key is not extractable');
        });
        it.each(['private', 'secret'] as KeyType[])('throws when a %s key is supplied', type => {
            ...
            expect(() => exportKeyPolyfill('raw', mockKey)).toThrow(`Unable to export a raw Ed25519 ${type} key`);
        });
    ...
    describe('when format is `pkcs8`', () => {
        it('throws when the key supplied is non-extractable', () => {
            ...
            expect(() => exportKeyPolyfill('pkcs8', mockKey)).toThrow('key is not extractable');
        });
        it.each(['public', 'secret'] as KeyType[])('throws when a %s key is supplied', type => {
            const mockKey = { extractable: true, type } as unknown as CryptoKey;
            expect(() => exportKeyPolyfill('pkcs8', mockKey)).toThrow(`Unable to export a pkcs8 Ed25519 ${type} key`);
        });
        it('throws when supplied a private key that was not generated with the polyfill', async () => {
            ...
            await expect(() => exportKeyPolyfill('pkcs8', privateKey)).toThrow();
        });
        it('returns the private key bytes associated with a secret key generated by the polyfill with pkcs8 header prepended', async () => {
            ...
            expect(exportKeyPolyfill('pkcs8', privateKey)).toEqual(expectedBytes);
        });
    });
});
...
describe('importKeyPolyfill', () => {
    describe('when format is `raw`', () => {
        ...
        it('imported key can be used to verify a signature', async () => {
            expect.assertions(1);
            const key = importKeyPolyfill('raw', MOCK_PUBLIC_KEY_BYTES, false, ['verify']);
            const result = await verifyPolyfill(key, MOCK_DATA_SIGNATURE, MOCK_DATA);
        ...
        it('imported key correctly does not verify a bad signature', async () => {
            ...
            const result = await verifyPolyfill(key, badSignature, MOCK_DATA);
        ...
        it('imported key can be exported to return the same bytes', async () => {
            ...
            const exported = await exportKeyPolyfill('raw', key);
        ...
        it('can import a public key that was exported from a polyfill generated key', () => {
            ...
            const publicKeyBytes = exportKeyPolyfill('raw', keyPair.publicKey);
            expect(() => importKeyPolyfill('raw', publicKeyBytes, false, ['verify'])).not.toThrow();
        });
    });
    ...
    describe('when format is `pkcs8`', () => {
        ...
        it('imported key can be exported to return the same bytes', async () => {
            ...
            const exported = await exportKeyPolyfill('pkcs8', key);
            expect(exported).toStrictEqual(mockSecretKeyWithHeader);
        });
        ...
        it('can import a private key that was exported from a polyfill generated key', async () => {
            ...
            const privateKeyPkcs8Bytes = exportKeyPolyfill('pkcs8', privateKey);
            expect(() => importKeyPolyfill('pkcs8', privateKeyPkcs8Bytes, false, ['sign'])).not.toThrow();
        });
    });
