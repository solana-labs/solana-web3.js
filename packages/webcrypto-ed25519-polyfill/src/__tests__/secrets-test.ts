...
describe('exportKeyPolyfill', () => {
    ...
    it('returns the public key bytes associated with a secret key generated by the polyfill', async () => {
        expect.assertions(1);
        const mockSecretKeyBytes = new Uint8Array([
            83, 147, 250, 112, 140, 37, 29, 73, 156, 38, 185, 76, 163, 8, 178, 225, 172, 53, 120, 108, 127, 191, 103, 8,
            160, 170, 183, 186, 246, 1, 227, 158,
        ]);
        jest.spyOn(globalThis.crypto, 'getRandomValues').mockReturnValue(mockSecretKeyBytes);
        const { publicKey } = generateKeyPolyfill(/* extractable */ false, ['sign', 'verify']);
        expect(exportKeyPolyfill('raw', publicKey)).toEqual(
            new Uint8Array([
                166, 132, 114, 186, 49, 163, 23, 12, 11, 14, 119, 219, 102, 96, 26, 226, 91, 97, 238, 217, 236, 84, 232,
                204, 62, 212, 179, 252, 20, 37, 179, 52,
            ])
        );
    });
});
