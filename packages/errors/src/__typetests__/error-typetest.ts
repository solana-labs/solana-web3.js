...
const { SOLANA_ERROR__TRANSACTION_FEE_PAYER_SIGNATURE_MISSING, SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING } =
...
const transactionMissingSignaturesError = new SolanaError(SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING, {
    addresses: ['123', '456'],
});

transactionMissingSignaturesError.context.__code satisfies typeof SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING;
// @ts-expect-error Wrong error code.
transactionMissingSignaturesError.context.__code satisfies typeof SOLANA_ERROR__TRANSACTION_FEE_PAYER_SIGNATURE_MISSING;

transactionMissingSignaturesError.context satisfies SolanaErrorContext[typeof SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING];
...
if (unknownError.context.__code === SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING) {
    unknownError.context satisfies SolanaErrorContext[typeof SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING];
    // @ts-expect-error Context belongs to another error code
    unknownError.context satisfies SolanaErrorContext[typeof SOLANA_ERROR__TRANSACTION_FEE_PAYER_SIGNATURE_MISSING];
}
...
if (isSolanaError(e, SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING)) {
    e.context satisfies SolanaErrorContext[typeof SOLANA_ERROR__TRANSACTION_SIGNATURES_MISSING];
    // @ts-expect-error Context belongs to another error code
    e.context satisfies SolanaErrorContext[typeof SOLANA_ERROR__TRANSACTION_FEE_PAYER_SIGNATURE_MISSING];
}
