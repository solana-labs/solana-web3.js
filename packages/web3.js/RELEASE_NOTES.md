...
- The legacy `Web3Signer` shape (`{publicKey, secretKey}`) and the `MessageSigner` type alias were removed. The exported `Signer` type is now `TransactionPartialSigner` from `@solana/kit`, and `VersionedTransaction.sign(...)` accepts an array of them plus an optional `{lastValidBlockHeight}` config. Pass `Keypair` instances or other Kit signers; migrate any remaining `{publicKey, secretKey}` literals via `await Keypair.fromSecretKey(legacySigner.secretKey)`.
...
- The exported `Signer` type is now `TransactionPartialSigner` from `@solana/kit`. The legacy `{publicKey, secretKey}` shape is no longer accepted by `Transaction.sign(...)`, `Transaction.partialSign(...)`, `VersionedTransaction.sign(...)`, `Connection.sendTransaction(...)`, `Connection.simulateTransaction(...)`, or `sendAndConfirmTransaction(...)`, and neither are message-only signers (`MessagePartialSigner`) or signers that modify or send the transaction themselves (`TransactionModifyingSigner`, `TransactionSendingSigner`). Transaction lifetime information is derived from the compiled message: a message whose first instruction is the System program's `AdvanceNonceAccount` instruction is signed with a durable nonce lifetime, and any other message is signed with a blockhash lifetime whose `lastValidBlockHeight` comes from the `VersionedTransaction.sign(...)` config or the legacy `Transaction`'s `lastValidBlockHeight` field, defaulting to the maximum possible value. A legacy `Transaction` with `nonceInfo` set always uses that nonce lifetime directly. Pass `Keypair` instances or other Kit signers; custom signers should implement Kit's `TransactionPartialSigner` shape.
